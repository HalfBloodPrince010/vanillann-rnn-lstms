...
class VanillaRNN(object):
    ...
    def step_backward(self, dnext_h, meta):
        ...
        # corresponding name.                                                       # 
        #############################################################################
        next_state, prev_h, x = meta  # tanh(Wax.X(t) + Waa.a(t-1) + bias)  -  state = next_h, that is a state
        dtanh = (1 - next_state**2)  # 1-tanh(x)^2
        dchain = dnext_h * dtanh  # Derivative is (dL/da * dx or dprev_h or dWh or dWx or db)
        dx = np.dot(dchain, self.params[self.wx_name].T)
        dprev_h = np.dot(dchain, self.params[self.wh_name].T)
        dWh = np.dot(prev_h.T, dchain)
        dWx = np.dot(x.T, dchain)
        db = np.sum(dchain, axis=0)
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
    ...
    def forward(self, x, h0):
        ...
        N, T, D = x.shape
        H = h0.shape[1]
        h = np.zeros((N, T, H))  # H for all time period for a batch size of N

        for t in range(T):
            if t == 0:
                h[:, t, :], meta_0 = self.step_forward(x[:, t, :], h0)
                self.meta.append(meta_0)
            else:
    ...
    def backward(self, dh):
        ...
        for t in range(T-1, -1, -1):
            dxt, dht, dWxt, dWht, dbt = self.step_backward(dnext_h, self.meta[t])
            dx[:, t, :] = dxt
            if t == 0:
                dh0 = dht
            else:
                dnext_h = dht + dh[:, t-1, :]  # Back propagation , state affects current Li, as well as si+1. So, two sources
...
class LSTM(object):
    ...
    def step_forward(self, x, prev_h, prev_c):
        ...
        #############################################################################
        N, H = prev_h.shape
        # a = Wx.prev_state + Wh.x + bias
        a = np.dot(x, self.params[self.wx_name]) + np.dot(prev_h, self.params[self.wh_name]) + self.params[self.b_name]
        # All gates, input(i), output(o), f(forget) and intermediate_state(g)
        # sigmoid(Different_Wx. prev_h + Different_Wh.x + bias)
        i = sigmoid(a[:, :H])
        f = sigmoid(a[:, H:2*H])
        o = sigmoid(a[:, 2*H:3*H])
        g = np.tanh(a[:, 3*H:])
        # curr_s ----> curr_h = tanh(curr_s) * output gate
        next_c = f * prev_c + i * g
        tnext_c = np.tanh(next_c)
        next_h = o * tnext_c
        meta = [self.params[self.wx_name], self.params[self.wh_name], i, f, o, g, tnext_c, prev_c, prev_h, x]
        #############################################################################
        #                               END OF YOUR CODE                            #
        #############################################################################
    ...
    def step_backward(self, dnext_h, dnext_c, meta):
        ...
        #############################################################################
        Wx, Wh, i, f, o, g, tnext_c, prev_c, prev_h, x = meta

        dnext_c += dnext_h * o * (1 - tnext_c ** 2)

        # Sigmoid derivative - sig(1-sig)
        d_out = tnext_c * dnext_h
        d_out_cap = ((1 - o) * o) * d_out
        ...
        d_g = i * dnext_c
        d_g_cap = d_g * (1 - (g ** 2))  # g = np.tanh(a[:, 3*H:])

        # print(d_g_cap.shape)
        da = np.hstack((d_i_cap, d_f_cap, d_out_cap, d_g_cap))
    ...
    def forward(self, x, h0):
        ...
        h = np.zeros((N, T, H))
        prev_h = h0
        t_cache = [None for i in range(T)]
        for t in range(T):
            next_h, next_c, t_cache[t] = self.step_forward(x[:, t, :], prev_h, prev_c)
            h[:, t, :] = next_h
    ...
    def backward(self, dh):
        ...
        for t in reversed(range(T)):
            dxt, dprev_h, dprev_c, dWxt, dWht, dbt = self.step_backward(dnext_h, dprev_c, t_meta[t])
            dx[:, t, :] = dxt
            if t == 0:
                dh0 = dprev_h
            else:
...
class word_embedding(object):
    ...
    def backward(self, dout):
        ...
        # Summation of gardients for all batches, if BATCH == 1, then update gradients of only that sequence
